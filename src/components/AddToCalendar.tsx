...
export function AddToCalendar({ variant = "primary", size = "md", className = "" }: AddToCalendarProps) {
  ...
  const buttonStyles = {
    primary:
      "bg-espresso text-white hover:opacity-90 border-espresso",
    secondary:
      "border-espresso/20 text-espresso bg-transparent hover:bg-espresso hover:text-white",
    "outline-white":
      "border-white/30 text-white bg-transparent hover:bg-white/10 hover:border-white/60",
  };

  const dropdownBg = {
    primary: "bg-white border-gray-200 shadow-lg",
    secondary: "bg-white border-gray-200 shadow-lg",
    "outline-white": "bg-espresso border-white/20",
  };

  const dropdownItem = {
    primary: "text-espresso hover:bg-gray-100",
    secondary: "text-espresso hover:bg-gray-100",
    "outline-white": "text-white hover:bg-white/10",
  };

  const sizeStyles = {
    sm: "px-5 py-2 text-[10px] tracking-[0.3em]",
  ...
  return (
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-full border font-bold uppercase transition-all duration-200 ${sizeStyles[size]} ${buttonStyles[variant]}`}
      ...
      {isOpen && (
        <div
          className={`absolute left-1/2 -translate-x-1/2 mt-2 w-56 rounded-lg border overflow-hidden z-20 ${dropdownBg[variant]}`}
        >
          {/* Google Calendar */}
          <a
            ...
            className={`flex items-center gap-3 px-4 py-3 transition-colors text-sm ${dropdownItem[variant]}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 w-full transition-colors text-sm ${dropdownItem[variant]}`}
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
